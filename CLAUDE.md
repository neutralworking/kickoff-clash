...
The Fork A rebuild target, now the LIVE game. Self-contained, deterministic, vitest-gated. Reuses the `VerbName` palette from `src/lib/verbs.ts` (unchanged; only targets re-point to contest dials). **P1 (NW-139)** landed the resolution spine; **P2 (NW-140)** the manager layer (reweight + tactical deck + adherence); **P3 (NW-141)** the card dataset + action catalogue + coverage + shop-bot; **P4 (NW-142)** the 9-fixture run loop + economy + modelled opponents + challenge rules. Four harnesses gate it: `six-contest.test.ts` (resolution/balance), `managers.test.ts` (reweight/swing/law/tactics/adherence), `cards.test.ts` (catalogue/coverage/draftability), and `run.test.ts` (the permadeath run-distribution — the "game works" gate). Balance instruments: `scripts/kc_v2_sim.ts` (match/manager), `scripts/kc_v2_runsim.ts` (run survival curve); `scripts/kc_v2_regenerate.ts` rebuilds the dataset + `docs/coverage_report_v2.md`. **The match is judged on the SCORELINE — win / draw / loss, nothing else (owner call, 2026-07; the Balatro points blind and its two scoring floors were REMOVED).** A win banks the full purse (`CASH_WIN + CASH_PER_GOAL×goals`), a draw survives on half (`DRAW_FACTOR`), a loss ends the run (v1 permadeath). Difficulty is carried by opponent quality growth (`OPP_BASE`/`OPP_GROWTH` in `run.ts` — tuned so it outpaces a draw-parker's half-purse deck growth) and the divergence lever is the economy (`K_QUALITY`: committed builds win more → compound faster). A possession build's route to survival is to SCORE: the chase drain (tired chasers) + **Build Pressure** (`manager.buildPressure` — consecutive held periods raise big-chance probability, commitment-gated; Metronome/Financier carry it). Settled shape at the current dials: committed ~30% completion vs uncommitted ~21%; **widening that gap and rebalancing the 11 managers under goals-only (attack managers currently outperform walls/possession ~60% vs ~11–26%) is the NEXT tuning campaign** — `scripts/kc_v2_runsim.ts` is the instrument. EVERY opponent is a **managed, modelled team** (CARD_SYSTEM_V2 §8): seeded manager + its posture + the reactive tactical brain (`autoTactics` — trailing chases, leading shuts the door); bosses additionally draft COMMITTED so their reweight fires; deny-chance actions are **bounded saves** so they defend without erasing the player's game.
...
| `managers.ts` (NW-140) | The 11-manager roster as DATA (no class): each is a committed-gated additive **reweight** package (`managerTraits`) + posture + formation + engine/variance/fitness/cash/`buildPressure` mechanics; `COMMIT_MIN` per contest; seeded `managerOffer` choice-of-three |
...
| `match.ts` | `simulateMatch` — the 6-batch × 3-increment loop ("period" in UI copy): manager reweight (folded into the possession split too) + adherence throttle + fitness → possession split → retain roll (KEEP↔BREAK) → CREATE→quality→xG→FINISH (Build Pressure: consecutive held batches raise pBig for committed possession managers) → set pieces → tactical windows (incl. commitment-gated dial boosts) + the `autoTactics` reactive brain (opponents) → the CHASE drain (`CHASE_DRAIN`: a KEEP-committed holder tires the chasing side each held batch) → subs → typed event log → **verdict = the scoreline (win/draw/loss)** |
...
| `data/challenges.ts` (NW-142) | The challenge-rule starter set (8) as data (opponent-quality modifiers / themed restrictions) + authored merge hook (empty until NW-147); `challengeForFixture` (seeded, from fixture 2) |
| `run.ts` (NW-142) | The 9-fixture run: `simulateRun`/`playFixture`/`fixtureSetup`/`resolveFixture` (the UI's entry points; `resolveFixture` takes the tactical-play schedule for interactive re-resolution), scoreline survival (win full purse / draw half / loss = permadeath), scaled managed opponents (every fixture; bosses draft committed), economy (cash → deck quality via `dialBonus`), serialize/resume |
