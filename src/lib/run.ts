...
// ---------------------------------------------------------------------------
// Opponent Builds — derived from the REAL engine XI
// ---------------------------------------------------------------------------
//
// The opponent you scout must be the opponent you play. The match engine fields a
// side generated by `generateOpponentXI(round, style, seed)` (opponent.ts); the
// scout build is now assembled from that *same* generated XI, so the formation,
// strength, named players, and star you see pre-match are the ones you face. Only
// the flavour metadata (team name, style label, the weakness soft-spot the engine
// rewards) is authored per round — the hand-written fake XIs of the fbal-era model
// are gone.

interface OpponentMeta {
  name: string;
  style: string;             // descriptive label (flavour only — engine uses the proto-style)
  actionsPerRound: number;
  synergies: string[];       // flavour
  weakness: string;          // SOFT SPOT shown to the player
  weaknessArchetype: string; // the archetype the engine actually rewards (match-v5)
}

const OPPONENT_META: OpponentMeta[] = [
  { name: 'FC Warm-Up',      style: 'Direct Play',    actionsPerRound: 0, synergies: [],                                          weakness: 'Weak to pace',                weaknessArchetype: 'Sprinter' },
  { name: 'Dynamo Midtable', style: 'Balanced',       actionsPerRound: 1, synergies: ['Engine Room'],                            weakness: 'Lacks creativity',            weaknessArchetype: 'Creator' },
  { name: 'Real Ambition',   style: 'Tiki-Taka',      actionsPerRound: 1, synergies: ['Puppet Masters', 'Passing Carousel'],     weakness: 'Overloaded midfield, open flanks', weaknessArchetype: 'Engine' },
  { name: 'AC Nightmare',    style: 'Counter-Attack', actionsPerRound: 2, synergies: ['Brick Wall', 'Fortress'],                 weakness: 'Compact but slow',            weaknessArchetype: 'Dribbler' },
  { name: 'The Invincibles', style: 'Gegenpressing',  actionsPerRound: 2, synergies: ['Pressing Trap', 'Shield & Sword', 'Creative Spark'], weakness: 'Relentless but fragile keeper', weaknessArchetype: 'Striker' },
];

/** A star's headline trick, keyed to their real archetype — so it always fits the
 *  player who actually carries it. Flavour only. */
const STAR_ABILITY: Record<string, string> = {
  Creator: 'Creates chances from nothing',
  Striker: 'Lethal in front of goal',
  Dribbler: 'Beats his man at will',
  Sprinter: 'Runs in behind all day',
  Target: 'Wins everything in the air',
  Passer: 'Dictates from deep',
  Controller: 'Never loses the ball',
  Engine: 'Covers every blade of grass',
  Destroyer: 'Wins everything in the tackle',
  Cover: 'Reads the game two moves ahead',
  Commander: 'Marshals the whole back line',
  Powerhouse: 'Bullies anyone in his path',
...
export function getOpponentBuild(cup: number, matchInCup: number, runSeed: number): OpponentBuild {
  ...
  const toDisplay = (c: Card): OpponentPlayer => ({
    ...
    power: c.power,
    personalityTheme: '', // generated opponents carry no personality theme
  });

  const xi = genXI.map(toDisplay);
  const baseStrength = xi.length
    ? Math.round(genXI.reduce((s, c) => s + c.power, 0) / genXI.length)
    : 0;

  // Star = the highest-rated outfielder (deterministic tiebreak by id); the keeper is
  // the star only if there is literally no one else.
  const byPower = [...genXI].sort((a, b) => b.power - a.power || a.id - b.id);
  const starCard = byPower.find(c => c.position !== 'GK') ?? byPower[0];
  const starPlayer = starCard ? toDisplay(starCard) : xi[0];
  const starAbility = starCard ? (STAR_ABILITY[starCard.archetype] ?? 'The danger man') : '';
  ...
  return {
    ...
    style: meta.style,
    baseStrength,
    actionsPerRound: meta.actionsPerRound,
    xi,
    synergies: meta.synergies,
    weakness: meta.weakness,
    weaknessArchetype: meta.weaknessArchetype,
    starPlayer,
    starAbility,
  };
}
