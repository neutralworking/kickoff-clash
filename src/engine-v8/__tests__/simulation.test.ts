...
describe('V8 prototype economy simulation', () => {
  it('replays deterministically for the same seeds and energy curve', () => {
    const first = simulatePrototypeBatch(80, [4, 6, 7, 9], 1000);
    const replay = simulatePrototypeBatch(80, [4, 6, 7, 9], 1000);
    expect(replay).toEqual(first);
  });

  it('keeps all three zones relevant after re-costing around natural zone power', () => {
    const result = simulatePrototypeBatch(120, [4, 6, 7, 9], 2000);
  ...
  it('creates playable future-period Chance cards rather than automatic chance resolution', () => {
    const result = simulatePrototypeBatch(120, [4, 6, 7, 9], 3000);
  ...
  it('makes the final period the largest scoring window under full-board period banking', () => {
    const result = simulatePrototypeBatch(120, [4, 6, 7, 9], 4000);
  ...
  it('shows the more generous late-energy curve as the more explosive comparison', () => {
    const controlled = simulatePrototypeBatch(120, [4, 6, 7, 9], 5000);
    const explosive = simulatePrototypeBatch(120, [4, 6, 8, 10], 5000);
