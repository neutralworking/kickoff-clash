...
interface SensitivityResult {
  ...
  directStepoverGenerations: number;
  pairings: Array<{
...
function deckFor(squad: V8CalibrationSquadKey): readonly string[] {
  return squad === 'dribbling_penalty' ? candidateDeck() : getV8CalibrationSquad(squad).playerIds;
}

function forceOkochaIntoAtt(
  state: V8CalibrationState,
  side: V8CalibrationSide,
  squad: V8CalibrationSquadKey,
  plays: readonly V8CalibrationMatrixPlay[],
): V8CalibrationMatrixPlay[] {
  if (squad !== 'dribbling_penalty') return [...plays];
  const existingAtt = calibrationPlayersInZone(state, side, 'ATT').length;
  const pendingAtt = plays.filter((play) => play.side === side
    && play.zone === 'ATT'
    && (play.kind === 'player' || play.kind === 'manager')).length;
  const canMoveOkocha = existingAtt + pendingAtt < 4;

  return plays.map((play) => (
    canMoveOkocha && play.kind === 'player' && play.side === side && play.cardId === 'okocha'
      ? { ...play, zone: 'ATT' as const }
      : play
  ));
}
...
function isDefender(state: V8CalibrationState, runtimeId: string): boolean {
...
function revealWithStepoverSensitivity(
  state: V8CalibrationState,
  side: V8CalibrationSide,
  cardId: string,
  zone: V8Zone,
  ...
  if (cardId !== 'okocha') {
    return { state: revealCalibrationPlayer(state, side, cardId, zone), directStepoverGenerations: 0 };
  }

  const opponent: V8CalibrationSide = side === 'home' ? 'away' : 'home';
  const target = calibrationPlayersInZone(state, opponent, opposingDepthZone(zone))
    .filter((player) => isDefender(state, player.runtimeId))
    .sort((a, b) => currentCalibrationDefence(state, a.runtimeId) - currentCalibrationDefence(state, b.runtimeId)
      || a.deployedOrder - b.deployedOrder || a.runtimeId.localeCompare(b.runtimeId))[0];
  const beforeDef = target ? currentCalibrationDefence(state, target.runtimeId) : undefined;
  const penaltiesBefore = calibrationHandTacticals(state, side).filter((card) => card.type === 'penalty').length;

  let next = revealCalibrationPlayer(state, side, cardId, zone);
  const penaltiesAfterReveal = calibrationHandTacticals(next, side).filter((card) => card.type === 'penalty').length;
  const reductionLanded = target !== undefined
    && beforeDef !== undefined
    && currentCalibrationDefence(next, target.runtimeId) < beforeDef;

  if (reductionLanded && penaltiesAfterReveal === penaltiesBefore) {
    next = addCalibrationTacticalToHand(next, side, 'penalty', { generatedBy: 'okocha' }).state;
    return { state: next, directStepoverGenerations: 1 };
  }

  return { state: next, directStepoverGenerations: 0 };
}

function resolveSequence(state: V8CalibrationState, plays: readonly V8CalibrationMatrixPlay[]): ResolvedSequence {
  let next = state;
  let directStepoverGenerations = 0;
  for (const play of plays) {
    if (play.kind === 'player') {
      const revealed = revealWithStepoverSensitivity(next, play.side, play.cardId, play.zone);
      next = revealed.state;
      directStepoverGenerations += revealed.directStepoverGenerations;
  ...
  return { state: next, directStepoverGenerations };
}
...
function simulate(homeSquad: V8CalibrationSquadKey, awaySquad: V8CalibrationSquadKey, seed: number) {
  let state = withCalibrationEnergy(createV8CalibrationMatch(
    seededShuffle(deckFor(homeSquad), seed),
    seededShuffle(deckFor(awaySquad), seed + 1),
  ));
  let homeScore = 0;
  let awayScore = 0;
  ...
  for (let periodIndex = 0; periodIndex < 4; periodIndex += 1) {
    ...
    const homePending = forceOkochaIntoAtt(away.state, 'home', homeSquad, home.pending);
    const awayPending = forceOkochaIntoAtt(away.state, 'away', awaySquad, away.pending);
    const plays = [...homePending, ...awayPending];
    ...
    directStepoverGenerations += firstResolved.directStepoverGenerations + secondResolved.directStepoverGenerations;
  ...
  return { homeScore, awayScore, penaltyResolutions, enhancedPenaltyResolutions, directStepoverGenerations };
}
  ...
  for (const opponent of opponents) {
    ...
    for (const seed of V8_CALIBRATION_MATRIX_SEEDS) {
      for (const dribblingHome of [true, false]) {
        ...
        directStepoverGenerations += match.directStepoverGenerations;
...
describe('V8 STEPOVER direct-generation sensitivity', () => {
  it('measures the compact standalone Okocha route without changing accepted gameplay', () => {
    ...
    writeFileSync(join(outputDir, 'v8-calibration-dribbling-stepover-sensitivity.json'), `${JSON.stringify(result, null, 2)}\n`);
    writeFileSync(join(outputDir, 'v8-calibration-dribbling-stepover-sensitivity.txt'), `${format(result)}\n`);
    console.log(`\n${format(result)}\n`);

    expect(result.matches).toBe(320);
    expect(result.directStepoverGenerations).toBeGreaterThan(0);
  }, 15_000);
});
